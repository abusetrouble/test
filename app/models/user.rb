...
class User < ActiveRecord::Base
  ...
  attr_accessible :avatar_id, :company_name, :country_id, :description, :email,
                  ...
                  :profile_public, :state_id, :stylesheet, :time_zone, :vendor, :zip, :avatar_attributes, :birthday, :family_id, :city_id, :rating
  ...
  def user_family
    Family.find_by_id(self.family_id).name
  end

  def user_city
    City.find_by_id(self.city_id).name
  end
