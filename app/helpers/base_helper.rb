...
module BaseHelper
  ...
  def user_knows_link(user = nil)
    html = "<span class='user_knows' id='user_knows_#{user.id}'>"
    html += link_to_remote :i_know_you.l,
                           { :update => "user_knows_#{user.id}",
                             :loading => "$$('span#user_knows_#{user.id} span.spinner')[0].show(); $$('span#user_knows_#{user.id} a.know_user_btn')[0].hide()",
                             #:loading => "$$('h4#user_rating')[0].hide()",
                             :complete => visual_effect(:highlight, "user_rating", :duration => 5),
                             :url => hash_for_know_url(:user_id => user.id),
                             :method => :post
                           }, {:class => "know_user button"}
    html +=	"<span style='display:none;' class='spinner'>"
    html += image_tag('spinner.gif')
    html += "#{:updating_rating.l} ...</span></span>"
    html.html_safe
  end
